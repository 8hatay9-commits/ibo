...
def patch_core(src):
    src = src.replace("FLASHBOT-PRODUCTION-V3.7.0","FLASHBOT-PRODUCTION-V3.8.3",1)

    old = 'workq=queue.Queue(maxsize=256); stop_evt=threading.Event()'
    new = 'workq=queue.Queue(maxsize=1); stop_evt=threading.Event()'
    if old not in src:
        raise RuntimeError("V3.8.3 queue marker not found")
    ...
    new = '''            merged=batch
            ...
            while True:
                try:
                    ...
                    break
            if stale:
                st["worker_coalesced_stale_batches_v383"]=int(st.get("worker_coalesced_stale_batches_v383",0))+stale
'''
    if old not in src:
        raise RuntimeError("V3.8.3 worker marker not found")
    src = src.replace(old,new,1)
...
'''
    new = '''                if fresh:
                    try:
                        ...
                        try:
                            workq.get_nowait()
                            st["feed_coalesced_batches_v383"]=int(st.get("feed_coalesced_batches_v383",0))+1
                        except queue.Empty:
                            pass
                        try:
                            workq.put_nowait(fresh)
...
'''
    if old not in src:
        raise RuntimeError("V3.8.3 producer marker not found")
    src = src.replace(old,new,1)

    old = '"pancakeswap_v3_enabled":True,"cost_gate":"V37_CONSERVATIVE_L1_L2_ESTIMATE"})'
    new = '"pancakeswap_v3_enabled":True,"cost_gate":"V37_CONSERVATIVE_L1_L2_ESTIMATE","queue_policy_v383":"LATEST_STATE_COALESCE_CAP1"})'
    if old not in src:
        raise RuntimeError("V3.8.3 status marker not found")
    src = src.replace(old,new,1)
    return src
...
def supervise():
    ...
    while True:
        if STOP.exists():
            write_state({
                "ok":True,"version":"FLASHBOT-SUPERVISOR-V3.8.3","status":"stopped",
        ...
        write_state({
            "ok":True,"version":"FLASHBOT-SUPERVISOR-V3.8.3","status":"running",
        ...
        write_state({
            "ok":False,"version":"FLASHBOT-SUPERVISOR-V3.8.3","status":"backoff",
            "supervisor_pid":os.getpid(),"child_pid":None,
