...
for url in cfg.get("flashblocks_wss_candidates",[]):
    item={"url":url,"ok":False,"events":0}; ws=None
    try:
...
try:
    rpc=RpcClient(cfg["flashblocks_http"],min_interval=0.7);samples=[]
    for _ in range(2):
...
except Exception as e:report["http_pending_logs"]={"ok":False,"error":f"{type(e).__name__}: {e}"}
...
print(json.dumps(report,indent=2));raise SystemExit(0 if report["ok"] else 4)
