...
def diag(obj):
    tmp=STATE.with_name(f"{STATE.name}.{os.getpid()}.v3102.tmp")
...
diag({"ok":False,"version":"FLASHBOT-PRODUCTION-V3.10.2","mode":"DRY_RUN_ONLY",
      "phase":"outer_worker_boot_v3102" if worker else "outer_supervisor_boot_v3102",
...
try:
    ...
    if bad not in src:
        raise RuntimeError("V3.10.2 bad blob-header marker not found")
        ...
        def patch_nested(nested_src):
            def patch_probe(probe_src):
                if "[25,100]" not in probe_src:
                    raise RuntimeError("V3.10.2 [25,100] probe marker not found")
    ...
    src=src.replace("V3.9.8","V3.10.2")
    compile(src,URL,"exec")
    exec(compile(src,URL,"exec"),{"__name__":"__main__","__file__":str(Path(__file__).resolve())})
except BaseException as e:
    try:
        diag({"ok":False,"version":"FLASHBOT-PRODUCTION-V3.10.2","mode":"DRY_RUN_ONLY",
              "phase":"outer_worker_error_v3102" if worker else "outer_supervisor_error_v3102",
