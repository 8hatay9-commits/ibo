...
EVENTS=ROOT/"events.ndjson";STRUCT=ROOT/"candidates.ndjson";QUOTED=ROOT/"quoted_candidates.ndjson"
...
def quote_2pool(q,c):
    p1,p2=c["pools"]
    if p1["venue"] not in cfg["quote"]["supported_venues"] or p2["venue"] not in cfg["quote"]["supported_venues"]:return []
    start=c["tokens"][0]
    dec=q.decimals(start);rows=[]
    for human in cfg["quote"]["sizes_human"]:
        amount=int(float(human)*(10**dec))
        if amount<=0:continue
        try:
            mid,t1,g1=q.one(p1,start,amount)
            out,t2,g2=q.one(p2,t1,mid)
            if t2.lower()!=start.lower():continue
            gross=out-amount;bps=(gross*10000.0/amount) if amount else -1e99
            rows.append({"human_in":human,"amount_in":str(amount),"amount_out":str(out),"gross_raw":str(gross),
                         "gross_edge_bps":bps,"gas_estimate_univ3":int((g1 or 0)+(g2 or 0))})
        except Exception as e:
            rows.append({"human_in":human,"error":f"{type(e).__name__}: {e}"})
    return rows
...
def handle_logs(st,logs,q,trigger=None):
    ...
    for p in touched:
        for c in cycles_from_pool(p):
            ...
            plain={"kind":c["kind"],"pools":[x["address"] for x in c["pools"]],"venues":[x["venue"] for x in c["pools"]],
                   "tokens":c["tokens"],"at":time.time(),"trigger_tx":tx,"block":block}
            with STRUCT.open("a",encoding="utf-8") as f:f.write(json.dumps(plain)+"\n")
            if cfg["quote"]["enabled"] and c["kind"]=="2pool":
                rows=quote_2pool(q,c);st["quote_attempts"]+=1
                good=[r for r in rows if "gross_edge_bps" in r and r["gross_edge_bps"]>=cfg["quote"]["min_gross_edge_bps"]]
                if good:
                    best=max(good,key=lambda x:x["gross_edge_bps"]);st["gross_positive_candidates"]+=1
                    rec=dict(plain);rec.update({"quotes":rows,"best":best,"net_profit_verified":False,
                                               "note":"gross quote only; flash premium/gas/L1/slippage atomic simulation gate not yet applied"})
...
def http_fallback(st,deadline,q):
    ...
    while not STOP.exists() and (not deadline or time.time()<deadline):
        ...
        try:
            logs=rpc.call("eth_getLogs",[{"fromBlock":"pending","toBlock":"pending"}],timeout=15,max_attempts=5) or []
            fresh=[]
            for lg in logs:
            ...
            st["messages"]+=1;st["last_event_at"]=time.time();handle_logs(st,fresh,q,{})
            st["pool_count"]=db.count_pools();st["last_error"]=None;st["backfill_active"]=ACTIVE.exists();save(st);time.sleep(pause)
        except Exception as e:
            st["last_error"]=f"{type(e).__name__}: {e}";save(st);time.sleep(min(20,max(3,pause*2)))
...
def main(seconds=0):
    ...
    st={"ok":True,"version":cfg["version"],"mode":cfg["mode"],"started_at":time.time(),"connected":False,"feed_mode":None,
        "messages":0,"txs":0,"logs":0,"known_pool_hits":0,"structural_candidates":0,"quote_attempts":0,
        "gross_positive_candidates":0,"pool_count":db.count_pools(),"last_error":None,"last_event_at":None}
