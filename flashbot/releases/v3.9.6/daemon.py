...
def write_diag(obj):
    tmp=STATE.with_name(f"{STATE.name}.{os.getpid()}.v396.tmp")
    tmp.write_text(json.dumps(obj,indent=2),encoding="utf-8")
    tmp.replace(STATE)

is_worker="--worker" in sys.argv
if is_worker:
    write_diag({"ok":False,"version":"FLASHBOT-PRODUCTION-V3.9.6","mode":"DRY_RUN_ONLY","phase":"wrapper_worker_boot","last_error":None,"timestamp":time.time()})
...
try:
    ...
    if is_worker:
        try:
            write_diag({"ok":False,"version":"FLASHBOT-PRODUCTION-V3.9.6","mode":"DRY_RUN_ONLY","phase":"wrapper_worker_error","last_error":f"{type(e).__name__}:{e}","traceback_tail":traceback.format_exc()[-3000:],"timestamp":time.time()})
        except Exception:
            pass
    raise
